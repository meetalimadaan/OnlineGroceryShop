        ...
        func updateQuantityInCart() {
            ...
            userCartRef.getDocument { [weak self] (document, error) in
                if let document = document, document.exists {
                    ...
                    if let index = cartItems.firstIndex(where: { $0["productID"] as? String == self?.product.id }) {
                        let newQuantity = self?.cartQuantity ?? 0
                        let productPrice = self?.product.price ?? 0
                        cartItems[index]["quantity"] = newQuantity
                        cartItems[index]["totalPrice"] = Double(newQuantity) * productPrice
                        
                        userCartRef.updateData(["cartItems": cartItems]) { error in
                            if let error = error {
                                print("Error updating quantity: \(error.localizedDescription)")
                            } else {
                                // Update the cartItems in MyCartViewModel
                                if let updatedCartItem = cartItems.first(where: { $0["productID"] as? String == self?.product.id }) {
                                    let newItem = CartItem(
                                        id: updatedCartItem["productID"] as? String ?? "",
                                        name: updatedCartItem["name"] as? String ?? "",
                                        price: updatedCartItem["price"] as? Double ?? 0.0,
                                        quantity: updatedCartItem["quantity"] as? Int ?? 0,
                                        img: updatedCartItem["img"] as? String ?? ""
                                    )
                                    
                                    if let index = MyCartViewModel.shared.cartItems.firstIndex(where: { $0.id == newItem.id }) {
                                        MyCartViewModel.shared.cartItems[index] = newItem
                                    }
                                }
                            }
                        }
